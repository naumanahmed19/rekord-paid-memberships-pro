...
*/
function rekord_pmprorh_signup_shortcode($atts, $content=null, $code="")
{
	...
	?>
		...
		<?php } else { ?>
		<form class="pmpro_form pmpro_signup_form form-material" action="<?php echo pmpro_url("checkout"); ?>" method="post">
			...
			<?php
				...
				else
				{
					...
					<?php input(['label'=>'Username','name'=>'username','size'=>30]); ?>
					<?php do_action("pmpro_checkout_after_username");?>
					<?php input(['label'=>'Password','name'=>'password','size'=>30, 'type'=>'password' ]); ?>
					<?php if($short) { ?>
						<input type="hidden" name="password2_copy" value="1" />
					<?php } else { ?>
						<?php input(['label'=>'Confirm Password','name'=>'password2','size'=>30, 'type'=>'password' ]); ?>
					<?php } ?>
					<?php do_action("pmpro_checkout_after_password");?>
					<?php input(['label'=>'E-mail Address','name'=>'bemail','size'=>30, 'type'=>'email' ]); ?>
					<?php if($short) { ?>
						<input type="hidden" name="bconfirmemail_copy" value="1" />
					<?php } else { ?>
						<?php input(['label'=>'Confirm E-mail','name'=>'bconfirmemail','size'=>30, 'type'=>'email' ]); ?>
					<?php } ?>
			...
			<div>
				<span id="pmpro_submit_span" >
					...
					<input type="submit" class="btn btn-primary btn-lg pmpro_btn pmpro_btn-submit-checkout" value="<?php echo $button; ?>" />
				</span>
			</div>
			<?php if(!empty($login) && empty($current_user->ID)) { ?>
			<div class="my-4">
				<a href="<?php echo wp_login_url(get_permalink()); ?>"><?php _e('Already have an account? Please Log In now','pmpro-register-helper'); ?></a>
			</div>
			<?php } ?>
		</form>
		<?php } ?>
...
add_shortcode("pmpro_signup", "rekord_pmprorh_signup_shortcode");
